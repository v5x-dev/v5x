# v5x

Tools for developing on the VEX V5 system, and for building your own tools on top of it.

Read the full documentation at [docs.v5x.dev](https://docs.v5x.dev).

## Packages

### [@v5x/cli](https://better-hub.com/v5x-dev/v5x/tree/main/packages/cli)

The command-line interface. Heavily inspired by [cargo-v5](https://better-hub.com/vexide/cargo-v5) in the vexide ecosystem, but it works for every type of v5 program.

### [@v5x/serial](https://better-hub.com/v5x-dev/v5x/tree/main/packages/serial)

The low-level foundation that powers the CLI: the V5 serial protocol implemented in TypeScript. Instead of shelling out to external binaries, it talks directly to the V5 brain over USB/serial.

This is a fork of [v5-serial-protocol](https://better-hub.com/LemLib/v5-serial-protocol) with extra features and fixes, major props to [jerrylum](https://better-hub.com/Jerrylum) and the [LemLib](https://better-hub.com/LemLib) team for making this whole project possible.

### [@v5x/web](https://better-hub.com/v5x-dev/v5x/tree/main/packages/web)

A browser workflow layer for v5 Web Serial applications. It wraps `@v5x/serial` with a small subscription-based client that tracks connection status, Web Serial support, and normalized errors. Framework bindings for React, Svelte, and Solid ship as subpath exports.

### [@v5x/events](https://better-hub.com/v5x-dev/v5x/tree/main/packages/events)

A strictly typed client for the VEX Events API v2. It covers events, teams, programs, seasons, matches, rankings, skills, and awards in browsers and server runtimes.

## Documentation development

```sh
bun run docs:check
bun run docs:export
```

Run `bun run docs:dev` to start the local Mintlify preview.
