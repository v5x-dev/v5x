...
When Web Serial is unavailable, snapshots use `status === "unsupported"` and expose a stable `unavailableReason`. See [Errors](/web/errors) for the complete unavailable reason list.
